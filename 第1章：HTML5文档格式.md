网页就是一个文本文件，文件扩展名一般为`.html`或`.htm`，俗称静态网页，可以直接在浏览器中预览；也可以是`.asp`、`.aspx`、`.php`或`.jsp`等，俗称为动态网页，需要服务器解析之后，浏览器才能够浏览。
...
文档结构大部分是使用`<div>`标签来完成的，为了能够识别不同的结构，一般通过定义`id`或`class`给它们赋予额外的语义。
